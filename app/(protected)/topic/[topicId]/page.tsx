...
export default function TopicPage() {
    ...
    useEffect(() => {
        getLevelStatus(topicId).then(setLevels);
    }, [topicId]);

    const handleStart = async (level: string) => {
        setLoadingLevel(level);
        try {
            const res = await generateQuestions(topicId, level);
                    ...
                    <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                        ...
                        <div className="glass px-6 py-3 rounded-2xl flex items-center gap-4">
                            <div className="text-right">
                                ...
                                <p className="text-white font-semibold">Level {levels.advanced ? '3' : levels.intermediate ? '2' : '1'} of 3</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 animate-slide-up">
                    {levelData.map((lvl) => (
                        ...
                        >
                            ...
                            <div className={`w-14 h-14 rounded-2xl mb-6 flex items-center justify-center border border-white/10 ${lvl.unlocked ? 'bg-white/5' : 'bg-transparent'}`}>
                                {lvl.icon}
                            </div>
                                ...
                                {lvl.unlocked ? (
                                    ...
                                    >
                                        {loadingLevel === lvl.id ? (
                                            ...
                                            lvl.score !== undefined ? `Retry (Best: ${lvl.score}%)` : "Start Level"
                                        )}
                                    </button>
                                ) : (
                            ...
                            {/* Decorative Shimmer for unlocked levels */}
                            {lvl.unlocked && (
