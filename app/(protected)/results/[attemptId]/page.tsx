...
export default async function ResultsPage({ params }: { params: Promise<{ attemptId: string }> }) {
  ...
  const topicName = typedData.topics?.[0]?.name || "Technical Assessment";
  ...
  return (
    <div className="min-h-screen px-6 py-12 bg-[#030712]">
      <div className="max-w-5xl mx-auto">
        <div className="mb-8">
          <Link href="/history" className="inline-flex items-center gap-2 text-slate-500 hover:text-white transition-colors group">
            ...
            <section className="glass rounded-[2rem] p-8 relative overflow-hidden group border-white/5">
              ...
              <ul className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {report?.strengths?.map((s, i) => (
                  <li
                    key={i}
                    className="flex gap-4 text-slate-300 bg-white/[0.03] p-5 rounded-2xl border border-white/5 hover:bg-white/[0.05] transition-colors"
                  >
